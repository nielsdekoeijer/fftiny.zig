...
// =utils======================================================================
// transpose 2x2
pub inline fn transpose2x2(comptime T: type, comptime mode: FFTMODE, x01: *const [4]T, x23: *const [4]T) [2][4]T {
    _ = mode;
    return [_][4]T{
        x01[0..2].* ++ x23[0..2].*,
        x01[2..4].* ++ x23[2..4].*,
...
test "transpose 2x2" {
    const T = f32;
    const x01 = [_]T{ 1.0, 2.0, 3.0, 4.0 };
    const x23 = [_]T{ 5.0, 6.0, 7.0, 8.0 };

    const y = transpose2x2(T, FFTMODE.FW, &x01, &x23);

    const x02 = [_]T{ 1.0, 2.0, 5.0, 6.0 };
    const x13 = [_]T{ 3.0, 4.0, 7.0, 8.0 };
    for (0..4) |i| {
...
pub inline fn rotateN90(comptime T: type, comptime mode: FFTMODE, x: *const [2]T) [2]T {
...
test "rotate 90" {
    const T = f32;
    const x01 = [_]T{ 1.0, 2.0 };
    const y01 = rotateN90(T, FFTMODE.FW, &x01);
    const z01 = [_]T{ 2.0, -1.0 };
    try std.testing.expect(y01[0] == z01[0]);
    try std.testing.expect(y01[1] == z01[1]);
}

// complex mul
pub inline fn cmul(comptime T: type, comptime x: [2]T, y: *const [2]T) [2]T {
    if (comptime (x[1] == 0.0)) {
        // ergo x[0] == + / 1 1.0...
        if (x[0] > 0.0) {
            return y.*;
...
test "cmul" {
    const T = f32;
    const x = comptime [_]T{ 1.0, 2.0 };
    const y = [_]T{ 3.0, 4.0 };
    const z = cmul(T, x, &y);
    const r = [_]T{ -5.0, 10.0 };
